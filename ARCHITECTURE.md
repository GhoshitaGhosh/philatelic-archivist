...
* **Chronological Context Node**: An `LlmAgent` tasked with deterministic grounding. Equipped with both the OCR text and full multimodal vision of the physical artifact, it automatically calls the `query_historical_database` tool (which uses a zero-blocking in-memory JSON cache) to link the artifact against verified milestones like the 1950 Republic Day or 1953 Everest issues. If critical historical data is missing, the node dynamically falls back to its native `google_search` tool (`GoogleSearchTool`) to execute a multimodal reverse-image search from the public web using Google's Knowledge Graph. Through strict prompt engineering, this node is explicitly mandated to extract exact dates and precise years, actively preventing vague LLM hallucinations and filling historical gaps efficiently.
