...
### 5. Native Multimodal Google Search Grounding
To prevent the catastrophic "telephone game" flaw where downstream nodes execute blind, text-only web searches, we integrated Native Google Search Grounding (`GoogleSearchTool`) directly into the ADK graph. Rather than deploying fragile third-party web scrapers that rely on C-extensions and frequently segfault inside Docker containers, the Gemini model uses its innate multimodal capabilities to execute authentic Reverse Image Searches through Google's infrastructure. It uses the restored physical image payload to query the web, perfectly disambiguating obscure historical variations without requiring hardcoded registries.
